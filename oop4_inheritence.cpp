...
class Developer : Employee{//Developer class inherits Employee and has acess to all its items
public:
    string favlang;

Developer(string name,int age, string company, string lang):Employee(name,company,age){
    favlang = lang;
} //Subclass needs to be referenced with the original class to get an instance

void fixer(){
    std::cout<<"Code has been fixed by "<<getnmae()<<" using "<<favlang<<endl;
}
...
int main(){
    ...
    Developer d = Developer("Rohan",19,"Microsoft","Python");
    d.fixer();
    return 0;
}